...
**AuthN/AuthZ**
- **Built (2026-07-21): ASP.NET Core Identity, JWT Bearer, not signed API keys.** `ApplicationUser : IdentityUser<Guid>`; `DaybookDbContext` extends `IdentityUserContext<ApplicationUser, Guid>` — deliberately not the role-bearing `IdentityDbContext`, since Identity's `IdentityRole` is a *global* RBAC concept and the per-book Owner/Editor/Viewer model below is structurally incompatible with it (it'll need a bespoke `BookMembership`-style join entity when built, not `AspNetRoles`). `AddIdentityCore<ApplicationUser>()`, not the full `AddIdentity<TUser,TRole>()` — the full variant wires cookie-based sign-in as the default scheme, which fights JWT Bearer as a second `AddAuthentication()` registration. `POST /v1/auth/register` is bootstrap-only (succeeds once, while zero users exist, then always rejects — an open registration endpoint on a self-hosted financial API is a real vulnerability surface for no v1 benefit). `POST /v1/auth/login` verifies via `UserManager.CheckPasswordAsync` and issues a real, inspectable JWT (not an opaque/proprietary token format) signed with a file-based HMAC key, mirroring the DB passphrase's own file-based secret convention. `ICurrentUser`'s real implementation (`CurrentUser`, reading the token's user-id claim via `IHttpContextAccessor`) lives in **Api**, not Infrastructure — the first port implementation in this codebase to do so, since `IHttpContextAccessor` is a web-framework type and auth is explicitly Api's own job per this doc's own architecture diagram. `GET /v1/me` proves the chain end to end and is a real, permanent endpoint (additive-only versioning applies to it too), not test scaffolding.
- Per-book roles: `Owner`, `Editor`, `Viewer`. Posting/reversing requires Editor+; reports require Viewer+. **Not yet built** — this milestone is authentication (who is this request from?), not authorization (what can they do?); role *enforcement* is still open work, per the "v1 doesn't build out multi-user management UI or flows" note below.
...
- Every request carries an authenticated principal; the acting user is stamped on every posted entry. **Not yet wired** — nothing in Core/Application yet calls `ICurrentUser` from around `Journal.Post`/`Reverse`; that's the still-parked first Application-layer use-case/composition-root milestone.

**Data protection**
- **In transit:** TLS required; HTTP disabled or redirected.
- **At rest:** encrypted SQLite via **SQLite3 Multiple Ciphers** (`SQLite3MC.PCLRaw.bundle`, swapped in for the originally-assumed SQLCipher path — see §13.5). The database key is derived from a passphrase via a KDF and protected with envelope encryption — never hard-coded, never in source. Full custody design is in §13.5.
...
- **SQLite provider: `SQLite3MC.PCLRaw.bundle`, not SQLCipher (resolved 2026-07-19).** The free prebuilt path §8 originally assumed — `SQLitePCLRaw.bundle_e_sqlcipher`/`bundle_e_sqlite3mc` — is dead: both deprecated upstream as of SQLitePCLRaw 3.0, frozen at 2.1.11 (March 2025), explicit "no longer maintained" notice. Rather than pin that dead bundle, pay for Zetetic's commercial SQLCipher-for-.NET package, or build a cipher-capable SQLite from source (all three real options, none of them good), a fourth path was found on closer research: the same maintainer who wrote `SQLite3MultipleCiphers` (an open-source, SQLCipher-compatible cipher engine, actively released — 2.3.6 as of 2026-07-12, tracking current SQLite 3.53.3) also publishes prebuilt .NET bindings under a **new** package family, `SQLite3MC.PCLRaw.{bundle,provider,lib}` — MIT licensed, monthly release cadence, and built through a genuinely serious supply chain (cross-repo build-provenance attestation, SignPath code signing) rather than an unaudited zip upload. Its release manifest includes prebuilt native binaries for `linux/x64`, `linux/arm64`, `linux/musl-x64`, and `linux/musl-arm64` — covering both glibc and musl (Alpine) container bases on both architectures, matching this project's "self-hosted, Linux-container-friendly" requirement directly. **Decision: use `SQLite3MC.PCLRaw.bundle`.** Free, actively maintained, no native build burden placed on this project. Everything else in this section — KDF, envelope encryption, passphrase-file handling — has no native dependency and was already built and tested independent of this choice. **The EF Core/`Microsoft.Data.Sqlite` wiring is now built** (2026-07-19): `EncryptedSqliteDbContextOptionsExtensions.UseEncryptedSqlite` (given an already-unwrapped data key + a database path) and `SqliteEncryptionKeyInterceptor`, which keys every physical connection via `PRAGMA cipher`/`PRAGMA hexkey` on open — deliberately not `SqliteConnectionStringBuilder.Password`, which funnels through SQLite's `quote()` on a TEXT-bound parameter and would silently re-derive a passphrase-based key from a raw hex string's literal characters instead of using the 32 raw bytes `Argon2Kdf`/`DataKeyEnvelope` already derived. Connection pooling is disabled so every open starts from a genuinely fresh, unkeyed native handle — `PRAGMA cipher`/`hexkey` "return ok" even for a wrong key, so a stale pooled key wouldn't necessarily fail loudly; a forced `SELECT count(*) FROM sqlite_master` immediately after keying turns a wrong key into an immediate thrown exception instead. Still takes the raw key as a parameter rather than deciding whether to generate or load a `WrappedDataKey` itself — that bootstrap decision remains composition-root work, not yet built.
...
- **Tamper-evidence via an anchored hash chain (in v1).** Each posted entry stores `hash(canonical_content + previous_entry_hash)`, following the gapless sequence, so any alteration or removal of history is detectable. **Built**: `JournalEntryCanonicalForm.Serialize` (a deterministic `BinaryWriter` encoding of every frozen-at-post field — deliberately excludes `LineTagLedger`/`ReconciliationLedger`, which are mutable after post); `JournalEntry.EntryHash`; `Journal.Post`/`Reverse` compute `HMAC-SHA256(chainKey, canonicalContent + previousHash)` and thread the running head hash forward (seeded correctly on `Rehydrate` from the highest-sequence posted snapshot, so a reloaded journal continues the same chain rather than restarting from genesis); `Journal.VerifyChain()` walks posted entries in sequence order and reports `Intact`/`NoChainKeyConfigured`/`ChainNotFullyPopulated`/`Tampered`, with the first affected entry. **Real finding**: "canonical serialization is already required for export and golden-journal tests" was aspirational, not true — no canonical-serialization or export code existed anywhere before this; it was built from scratch for the chain alone. **Deliberately narrowed**: the chain key is a construction-time, *optional* parameter on `Journal.Empty`/`Rehydrate` (default `null`, chaining disabled — today's behavior), not "built in from day one, always on" as originally framed — nothing in the codebase can yet derive/supply a real key, since no Application-layer use-case/composition-root wiring exists (the same gap flagged for `IClock`/`ICurrentUser`/`IAuditLog`). Requiring a key on every call site would force updating tests across the whole suite for a key nothing can produce for real yet. **Deferred**: emitting a signed head hash alongside backups (no backup/export feature exists yet) and persistence of `EntryHash` (Core-only this round, same precedent as every domain feature's own follow-up persistence slice).
